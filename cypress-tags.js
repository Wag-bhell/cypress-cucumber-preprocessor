...
const debug = (message, ...rest) =>
    process.env.DEBUG ? // eslint-disable-next-line no-console
...
const featuresToRun = paths.filter((featurePath) => {
    ...
    //****** CARREFOUR ZONE *****************
    const specl = `${fs.readFileSync(featurePath)}`.split("\r\n");
    //parse required tags
    var allTags = [envTags] || [""]
    if (envTags) {
        allTags = envTags.toLowerCase().split(" and ")
    }

    for (var i = 0; i < specl.length; i++) {
        //Check for feature tags match
        if (specl[i].match(/^ *Feature:/g)) {
            if (specl[i - 1] != undefined) {
                if (specl[i - 1].trim().toLowerCase().split(/ +/).some(x => allTags.includes(x))) {
                    //then the feature should run
                    return true;
                }
            }
        }
        //Check for Scenario tags match
        if (specl[i].match(/^ *Scenario ?(Outline)?:/g)) {
            if (specl[i - 1].match(/^ *@[a-zA-Z0-9_.-]*/g)) {
                if (specl[i - 1].trim().toLowerCase().split(/ +/).some(x => allTags.includes(x))) {
                    //then the feature should run
                    return true;
                }
...
try {
    if (featuresToRun.length || envTags === "") {
        execFileSync(
            getCypressExecutable(), [...stripCLIArguments(["g", "glob"]), "--spec", featuresToRun.join(",")], {
